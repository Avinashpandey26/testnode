...
module.exports = function(app, passport,server) {
	...
	app.get('/password', auth, function(request, response) {
		//var old = request.session.email;
        		...
        		if (user)
        			//console.log(req);
        		       req.body.Password = bcrypt.hashSync(req.body.Password, bcrypt.genSaltSync(8), null);
        				//req.body.Password = req.generateHash(req.body.Password);
            			user.updatePassword(req, res);
                 });
  		});
	/*app.post('/editpassword', passport.authenticate('password', {
	...
	*/
	...
	app.get('/logout', function(request, response) {
		...
		app.post('/edit',  function (req, res){
			//console.log(req);
				/* var tempPath = req.files.thefile.path,
        			targetPath = path.resolve('./uploads/'+req.files.thefile.originalFilename);
    				if (path.extname(req.files.thefile.name).toLowerCase() === '.png') {
